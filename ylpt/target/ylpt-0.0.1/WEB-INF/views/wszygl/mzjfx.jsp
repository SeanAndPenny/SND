...
<script type="text/javascript">
function getechars03(jgdm,kssj,jssj){
	    $.ajax({
	      url: "${ctx}/mzjfx/getechars03.json?p_jgdm="+jgdm
	            +"&p_kssj="+kssj
	            +"&p_jssj="+jssj,    
	       datatype:"json",
	      context: document.body, 
...
function getechars04(jbflbm,jgdm,kssj,jssj,name){ 
    $.ajax({
      url: "${ctx}/mzjfx/getechars04.json?p_jbflbm="+jbflbm
            +"&p_jgdm="+jgdm
            +"&p_kssj="+kssj
            +"&p_jssj="+jssj,    
       datatype:"json",
      ...
      success: function(data){
         ...
         for (var i=0;i<fianl.length;i++) { 
        	 rs[i]=fianl[i].rs;
        	 jbbm[i]=fianl[i].jzzdbm; 
        	 jbmc[i]=fianl[i].jzzdmc; 
            }
         
        chart04('chart04',rs,jbbm,jbmc,name);  
...
<script type="text/javascript">
 function chart02(id,yljgmc,mzrs,yljgdm,kssj,jssj){
	 ...
	 var myChart = echarts.init(document.getElementById(id));
	  ...
	  var option = {
	    ...
	    myChart.setOption(option)
	     myChart.on('click', function(params) {  
	    	 /* window.open(
	    			 "${ctx}/mzjfx/getchart03?p_yljgdm="+yljgdm[params.dataIndex] 
