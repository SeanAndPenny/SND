...
import com.wondersgroup.wszygl.manager.ZybasyManger;
import com.wondersgroup.wszygl.model.BasycxModel;
@Controller
@RequestMapping("/zybasy")
public class ZybasyController extends BaseMybatisController<BasycxModel,String>{
	@Autowired 
	public ZybasyManger zybasyManger;
	...
	public ModelAndView basyxzList(HttpServletRequest request, HttpServletResponse response) {
		...
		result.setViewName("wszygl/zybasy");
		try {
			result.addObject("kssj", kssj);
			result.addObject("jssj", jssj);
			result.addObject("jgdm", jgdm);
			PageRequest pageRequest = newPageRequest(request);
			PageResult<BasycxModel> page = zybasyManger.pageSelect(pageRequest);
