...
function getechatsdata(yljgdm,startdate,enddate){
    $.ajax({
    	url: "${ctx}/ff/fyfmecharts.json?p_yljgdm="+yljgdm+"&p_startdate="+startdate+"&p_enddate="+enddate,
    	 datatype:"json",
...
function getData(){
        layui.use(['table','layer','laydate'] , function(){
            ...
            table.render({
                ...
                cols: [[
                	{field: 'hzxm', title: '姓名', width: '80'},
                    {field: 'sfzh', title: '身份证号', width: '180'},
                    {field: 'csrq', title: '出生日期', width: '100'},
                    {field: 'lxdh', title: '联系电话', width: '120'},
                    {field: 'childbirthtime', title: '分娩日期', width: '130'},
                    {field: 'cdoctorreporgname', title: '分娩机构', width: '150'},
 					{field: 'cccfs', title: '分娩方式', width: '100'},
                    {field: 'cdoctorname', title: '分娩医生', width: '80'}, 
                    {field: 'cxb', title: '婴儿性别', width: '80'},
                    {field: 'isg', title: '婴儿身高', width: '100'},
                    {field: 'itz', title: '婴儿体重', width: '100'},
        ...
        //监听工具条
            table.on('tool(querytab)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'detail') {
					layer.msg('sfzh：'+ data.sfzh + ' 的查看操作');
					<!-- <a  href='fyfmxx?sfzh=data.sfzh' ></a> -->
				}
			});
        });
    }
