		...
		a.layTool.on("click", "*[lay-event]", function(e) {
			...
			switch(layui.stope(e), R.trigger("table.tool.panel.remove"), l.close(a.tipsIndex), c) {
				...
				case "LAYTABLE_PRINT":
					...
					v.append(a.layMain.find("table").html()), v.find("th.layui-table-patch").remove(), 
					v.find(".layui-table-col-special").remove(), h.document.write(f + v.prop("outerHTML")), h.document.close(), h.print(), h.close()
			}
